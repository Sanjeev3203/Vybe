...
const Security = (() => {
  ...
  const RATE_LIMITS = {};
  const SESSION_KEY = 'synchro_session_v1';
  const SALT_KEY = 'synchro_salt_v1';
  ...
  async function deriveKey(salt) {
    ...
    const keyMaterial = await crypto.subtle.importKey(
      'raw', enc.encode('synchro-client-key-2025'), 'PBKDF2', false, ['deriveKey']
    );
  ...
  function safeStore(key, value) {
    try {
      localStorage.setItem('synchro_' + key, JSON.stringify(value));
      return true;
    } catch (e) {
  ...
  function safeRead(key, fallback = null) {
    try {
      const raw = localStorage.getItem('synchro_' + key);
  ...
  function safeDelete(key) {
    try { localStorage.removeItem('synchro_' + key); } catch (e) {}
  }
...
window.__synchroSession = Security.init();
